...
% Create the matrices associated with each Fourier component in lead (left or right)
...
for nt1 = 1:1:n_tran1 % loop over Fourier components (transverse 1)
    for nt2 = 1:1:n_tran2 % loop over Fourier components (transverse 2)
        kvec = double(nt1)/double(n_tran1)*gvec1 ... % corresponding transverse wave vector
               + double(nt2)/double(n_tran2)*gvec2;
        % disp([size(kvec) kvec]);

        % kvec = kvec*(1+1i*1E-5); % DEBUG LINE
        nrange0 = TransCell(1).nrange;
        rvec0 = TransCell(1).rvec;
