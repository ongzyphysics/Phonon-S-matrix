...
% This function calculates the phonon transmission for frequency range wvec
...
t_L = 2*1i*w*sqrt(V_plus_R)*inv(U_plus_R)*EffG_RL* ... 
    inv(U_minus_L_adv')*sqrt(V_minus_L_adv)/sqrt(a_L*a_R);
% ===== (reflection matrix) ===
...
r_L = 2*1i*w*sqrt(V_minus_L)*inv(U_minus_L)*(EdgeG-EdgeG0)* ...
    inv(U_minus_L_adv')*sqrt(V_minus_L_adv)/sqrt(a_L*a_L);
...
t_R = 2*1i*w*sqrt(V_minus_L)*inv(U_minus_L)*EffG_LR* ... 
    inv(U_plus_R_adv')*sqrt(V_plus_R_adv)/sqrt(a_R*a_L);
% ===== (reflection matrix) ===
...
r_R = 2*1i*w*sqrt(V_plus_R)*inv(U_plus_R)*(EdgeG-EdgeG0)* ...
    inv(U_plus_R_adv')*sqrt(V_plus_R_adv)/sqrt(a_R*a_R);
