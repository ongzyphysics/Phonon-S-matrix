...
% matrices.
  ...
  t_RL = 2*1i*w*sqrt(V_plus_R)*inv(U_plus_R)*EffG_RL* ... 
  ...
  r_LL = 2*1i*w*sqrt(V_minus_L)*inv(U_minus_L)*(EdgeG-EdgeG0)* ...
  ...
  t_LR = 2*1i*w*sqrt(V_minus_L)*inv(U_minus_L)*EffG_LR* ... 
  ...
  r_RR = 2*1i*w*sqrt(V_plus_R)*inv(U_plus_R)*(EdgeG-EdgeG0)* ...
